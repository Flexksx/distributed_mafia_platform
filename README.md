...
The Mafia Game Platform is fully operational and ready for use. All services have been integrated and tested successfully, including the `message-broker` integration with the `rumors-service` and `communication-service`. The platform is now capable of handling real-time communication and event-driven interactions seamlessly.

## Key Highlights

- **Message Broker Integration**: The `message-broker` has been successfully integrated with the `rumors-service` and `communication-service`, enabling efficient communication via HTTP/gRPC.
- **Dockerized Services**: All services are containerized using Docker and orchestrated with Docker Compose for easy deployment.
- **Environment Configuration**: The `.env` file has been updated with all necessary environment variables to ensure smooth operation.
- **Testing and Validation**: Comprehensive testing has been conducted to verify the functionality of all services and their interactions.

## Next Steps

- Deploy the platform to your preferred environment.
- Monitor the services using the provided logs and health check endpoints.
- Extend the platform with additional features as needed.

For detailed instructions on setting up and running the platform, refer to the individual service README files.
...
**Purpose:** Dynamic stock management based on player actions and game state
...
8. For cosmetic items, notify Character Service
...
flowchart LR
    ...
    RS -- "Validates User" --> UMS
    RS -- "Reads/Writes roles, actions, announcements, player status" --> DB
    RS -- "POST /api/v1/users/{id}/transactions (SUBTRACT)" --> UMS
    UMS -- "GET /v1/users/{id}/balance" --> RS
    RS -- "POST /api/v1/actions/verify-item" --> CS
    CS -- "Item effectiveness response" --> RS
    RS -- "GET /v1/tasks/verify-item" --> TS
    TS -- "Task requirement validation" --> RS
    RS -- "POST /v1/users/{userId}/inventory (asset purchases)" --> CS
    CS -- "Asset added confirmation" --> RS
    GS -- "GET /api/v1/items/stock-status (game state)" --> RS
    RS -- "Active protection effects" --> GS
